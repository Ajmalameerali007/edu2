 ...
 * This prevents runtime crashes like: Cannot read properties of undefined (reading 'filter'/'includes')
 */
function normalizeDB(input: any): Database {
  ...
  db.settings = {
    ...base,
    ...db.settings,
    decoderRules: { ...base.decoderRules, ...(db.settings?.decoderRules ?? {}) },
    communicationControls: {
      ...base.communicationControls,
      ...(db.settings?.communicationControls ?? {}),
      blockedKeywords: asArray<string>((db.settings as any)?.communicationControls?.blockedKeywords),
    },
    sessionPolicies: { ...base.sessionPolicies, ...(db.settings?.sessionPolicies ?? {}) },
    templates: {
      ...base.templates,
      ...(db.settings?.templates ?? {}),
      homework: asArray((db.settings as any)?.templates?.homework),
      reports: asArray((db.settings as any)?.templates?.reports),
      tutorNotes: asArray((db.settings as any)?.templates?.tutorNotes),
    },
    branding: { ...base.branding, ...(db.settings?.branding ?? {}) },
  };
...
export function getUserByEmail(email: string): User | undefined {
  ...
  return db.users.find((u) => (u.email || "").toLowerCase() === safeEmail.toLowerCase());
}
...
export function createMessage(
  ...
  const body = typeof (messageData as any).body === "string" ? (messageData as any).body : "";
  ...
  const blockedKeywords = Array.isArray(settings?.communicationControls?.blockedKeywords)
    ? settings.communicationControls.blockedKeywords.filter((x) => typeof x === "string")
    : [];
